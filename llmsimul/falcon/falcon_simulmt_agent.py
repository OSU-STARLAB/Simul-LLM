    ...
    def policy(self):
        ...
        if lagging >= self.waitk or self.states.source_finished:
            if self.nmt_prompt and self.decoding_strategy == "greedy":
                ...
                model_output = self.make_inference_translation(current_source, current_target, num_beams=self.num_beams, num_chunks=self.num_chunks, window_size = self.window_size)
