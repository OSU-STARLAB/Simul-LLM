...
from llmsimul.bloom.bloom_stopping_criteria import StopTokenAndMaxLengthCriteria
...
@entrypoint
class BloomTextAgent(BasicLLMTextAgent):
    ...
    def make_inference_translation(self, source, current_translation, num_beams=1, num_chunks=1, window_size=10):
        ...
        if self.nmt_prompt:
            input_prompt = f'Translate from {self.source_lang} to {self.target_lang}: {{{source}}}#\nAssistant: {current_translation}'
        else:
            input_prompt = f'Given the {self.source_lang} sentence {{{source}}}, and the current translation in {self.target_lang} {{{current_translation}}}, what\'s the next translated word? #\nAssistant: '
